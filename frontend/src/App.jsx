...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-blue-500/30 overflow-x-hidden">
      ...
      {/* Top Navigation Bar - Authentic SaaS Look */}
        ...
        <div className="max-w-[1400px] mx-auto px-8 h-20 flex items-center justify-between">
          <div className="flex items-center gap-10">
            ...
            >
              ...
              <span className="font-extrabold text-2xl tracking-tighter">GRIDLOCK</span>
            </div>
            {/* Nav Links */}
          ...
          <div className="flex items-center gap-6 text-[#888]">
            ...
            <div className="flex items-center gap-3 cursor-pointer hover:bg-[#111] p-1.5 pr-4 rounded-full border border-transparent hover:border-[#333] transition-all">
              <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-500 to-indigo-600 border-2 border-black"></div>
      ...
      <AnimatePresence mode="wait">
        {appState === 'landing' ? (
          ...
          >
            ...
            {/* Hero Image Mockup */}
            <div className="mt-24 w-full rounded-2xl border border-[#333] bg-[#0a0a0a] p-4 shadow-2xl relative overflow-hidden group cursor-pointer" onClick={() => setAppState('dashboard')}>
          ...
          >
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-4">
              <div>
                <h2 className="text-4xl font-bold tracking-tight mb-2">Vision Pipeline</h2>
                <p className="text-[#888] text-lg font-medium">Upload images to run YOLOv8 object detection and OCR analysis.</p>
              </div>
              <div className="flex items-center gap-4">
                <span className="flex items-center gap-2 text-sm font-bold bg-emerald-500/10 text-emerald-400 px-4 py-2 rounded-lg border border-emerald-500/20 shadow-[0_0_15px_rgba(16,185,129,0.15)]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
              <StatCard title="Frames Processed" value="24.8k" trend="+12% today" icon={Database} colorClass="blue" />
              <StatCard title="Violations Detected" value="1,402" trend="+5% today" icon={AlertTriangle} colorClass="red" />
              <StatCard title="Average Confidence" value="94.2%" trend="Stable" icon={Shield} colorClass="emerald" />
              <StatCard title="Inference Speed" value="42ms" trend="-2ms avg" icon={Zap} colorClass="purple" />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-5 flex flex-col gap-8">
                <div className="bg-[#0a0a0a] border border-[#222] rounded-3xl overflow-hidden flex flex-col shadow-2xl">
                  <div className="px-8 py-6 border-b border-[#222] bg-[#0d0d0d] flex justify-between items-center">
                  ...
                  <div className="p-8 flex-1 flex flex-col">
                    {!preview ? (
                      <label className="flex-1 border-2 border-dashed border-[#333] rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-blue-500 hover:bg-blue-500/5 transition-all min-h-[350px] group">
                        <input type="file" className="hidden" accept="image/*" onChange={handleFileChange} />
                        <div className="w-20 h-20 bg-[#111] rounded-full flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                          <UploadCloud size={40} className="text-[#666] group-hover:text-blue-500 transition-colors" />
                        </div>
                        <p className="text-xl font-bold text-white mb-2">Drag & Drop Image</p>
                        <p className="text-[#666] font-medium">or click to browse local files</p>
                      </label>
                    ) : (
                      <div className="flex-1 flex flex-col">
                        <div className="relative rounded-2xl overflow-hidden border border-[#333] bg-[#050505] flex items-center justify-center min-h-[300px] group">
                          <img src={preview} alt="Upload Preview" className="max-w-full max-h-[400px] object-contain" />
                          <label className="absolute top-4 right-4 bg-black/80 backdrop-blur-md border border-[#444] text-white text-sm font-semibold px-4 py-2 rounded-xl cursor-pointer hover:bg-black transition-colors opacity-0 group-hover:opacity-100">
                            <input type="file" className="hidden" accept="image/*" onChange={handleFileChange} />
                            Replace File
                          </label>
                        </div>
                        ...
                        <button 
                          ...
                          disabled={loading}
                          className="mt-8 w-full bg-white text-black font-extrabold text-lg py-5 rounded-2xl hover:bg-zinc-200 hover:scale-[1.02] transition-all disabled:opacity-50 disabled:hover:scale-100 flex items-center justify-center gap-3 shadow-[0_0_30px_rgba(255,255,255,0.15)]"
                        >
                          {loading ? (
                          ...
                          {loading ? 'Processing via YOLOv8...' : 'Run Traffic Analysis'}
                        </button>
                      </div>
                    )}
                    {error && (
              ...
              <div className="lg:col-span-7 flex flex-col">
                <div className="bg-[#0a0a0a] border border-[#222] rounded-3xl overflow-hidden flex flex-col shadow-2xl min-h-[600px] h-full">
                   <div className="px-8 py-6 border-b border-[#222] bg-[#0d0d0d] flex items-center justify-between">
                    <h2 className="text-lg font-bold flex items-center gap-3">
                      <Monitor className="text-emerald-500" size={22} />
                      AI Telemetry Output
                    </h2>
                    {result && (
                      <span className="text-xs font-bold bg-[#111] border border-[#333] text-[#888] px-3 py-1.5 rounded-lg tracking-wider uppercase">
                        {result.report.violations.length} Matches Found
                      </span>
                    )}
                  </div>
                  ...
                  <div className="p-8 flex-1 flex flex-col bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#111] via-[#0a0a0a] to-[#0a0a0a]">
                    {!result && !loading ? (
                      <div className="flex-1 flex flex-col items-center justify-center text-[#444]">
                        ...
                        <p className="text-lg font-semibold">Awaiting image input to start telemetry.</p>
                      </div>
                    ) : loading ? (
                      <div className="flex-1 flex flex-col items-center justify-center">
                        ...
                        <p className="text-xl font-bold text-white mb-2 tracking-wide">Analyzing Frame</p>
                        <p className="text-[#888] font-mono text-sm">Running object detection and OCR...</p>
                      </div>
                    ) : (
                      <motion.div 
                      ...
                      >
                        {/* Annotated Image */}
                        <div className="rounded-2xl overflow-hidden border border-[#333] bg-[#050505] flex justify-center shadow-[0_0_40px_rgba(0,0,0,0.5)]">
                          <img 
                        ...
                        <div className="flex-1">
                          <h3 className="text-sm font-bold text-[#888] uppercase tracking-widest mb-4 flex items-center gap-2">
                            <FileText size={16} /> Data Payload
                          </h3>
                          
                          {result.report.violations.length > 0 ? (
                            <div className="border border-[#222] rounded-2xl overflow-hidden bg-[#0d0d0d]">
                                    ...
                                    <tr key={i} className="hover:bg-[#151515] transition-colors">
                                      ...
                                      <td className="px-6 py-5 text-white font-semibold capitalize flex items-center gap-3">
                                        ...
                                        {v.plate && v.plate !== "UNKNOWN" && (
                                          <span className="font-mono text-xs bg-blue-500/10 text-blue-400 border border-blue-500/20 px-2 py-1 rounded-md">
                                            {v.plate}
                                          </span>
                                        )}
                                      </td>
                                      <td className="px-6 py-5 text-right">
                                        <span className="font-mono text-emerald-400 font-bold bg-emerald-400/10 px-2 py-1 rounded-md">
